<%-- 
    Document   : viewcart
    Created on : Apr 21, 2015, 11:16:16 PM
    Author     : Peter
--%>
...
<html>
    <head>
        ...
        <title>E-Commerce View Cart Page</title>
    </head>

    <body onload="http://yourservername:port/app/Controller">
    ...
    <c:forEach var="logindb" items="${listofLogins.rows}">
        <h2>You are currently login as: "<u>${logindb.lgname}</u>"</h2>
...
<!--                <th>Item Action</th>-->
    ...
    <img src="<c:out value="${cartdb.cimage}" />"  style="max-height:100%; max-width:100%"/>   
        ...
        <br>
        <button type="Submit" name="Enter" value="checkout" onclick="document.getElementById('checkout.jsp').submit()"/>Checkout</button>
    
    </form>
        
        <form>
            <br>
        <button type="Submit" name="Enter" value="clearcart" />ClearCart</button>
        </form>
